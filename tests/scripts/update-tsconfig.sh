...
assert_json "has next plugin" "_.compilerOptions.plugins.some(p => p.name === 'next')" "./tsconfig.json"
assert_json "has @/* path alias" "_.compilerOptions.paths['@/*'][0] === './*'" "./tsconfig.json"
...
assert_json "include has .next/types/**/*.ts" "_.include.includes('.next/types/**/*.ts')" "./tsconfig.json"
